...
def main():
    ...
    uni_label_id = [0, 1, 2, 3, 4, 5]
    uni_label = ["CPR:3", "CPR:4", "CPR:5", "CPR:6", "CPR:9", "false"]
    ...
    df = evl.get_df(test_file, '\t', header='infer')
    df['predicted_label'] = res_label
    df.to_csv(os.path.join(output_path,'clinical_data_pred_chemprot.tsv'), index=False, sep='\t', encoding='utf=8')
