...
blue_bert_dir = os.environ['BlueBERT_DIR']
vocab_file = os.path.join(blue_bert_dir, 'vocab.txt')
tokenizer = tokenization.FullTokenizer(vocab_file=vocab_file, do_lower_case=True)
drug_tokens = ['@', 'drug', '$']#'@drug$' # '@DRUG$' due to lower_case trained model
#'@drug$' # '@DRUG$'


def tokens_pos(tokens:list, drug_tokens:list = drug_tokens):
    pos = [i for i in range(1, len(tokens)-1) if tokens[i-1]==drug_tokens[0] and tokens[i]==drug_tokens[1] and tokens[i+1]==drug_tokens[2]]
    if len(pos)==2:
        token1_pos, token2_pos = pos[0], pos[1]
    elif len(pos)==1:
        token1_pos, token2_pos = pos[0], -1
    else:
        token1_pos, token2_pos = -1, -1
    return (len(pos), token1_pos, token2_pos)
...
def len_token(text):
    ...
    pos_length, token1_pos, token2_pos =  tokens_pos(tokens, drug_tokens)
    return (len(tokens), pos_length, token1_pos, token2_pos)
...
def replace_string(text, ann1_start, ann1_end, ann2_start, ann2_end, replace_string='@DRUG$'):
        ...
        if 'drug' in js_obj['entities']:
            if len(js_obj['entities']['drug'])>=2:
                ...
                for pair in pairs_combination:
                    text = js_obj['title'] + ' ' + js_obj['abstract'] # title, space and abstract
                    drug_1 = text[pair[0][0]:pair[0][1]]
                    drug_2 = text[pair[1][0]:pair[1][1]]
                    if drug_1!=drug_2:
                        tmp_dict = {}
                        tmp_dict['pmid']= js_obj['pmid']
                        # tmp_dict['text'] = text # space between title and abstract
                        tmp_dict['text_@DRUG$'] = replace_string(text, pair[0][0], pair[0][1], pair[1][0], pair[1][1])
                        # print (text)
                        # print (tmp_dict['text_@DRUG$'])
                        token_length, pos_length, token1_pos, token2_pos= len_token(tmp_dict['text_@DRUG$'])
                        if pos_length==2:
                            tmp_dict['token_length']= token_length
                            # tmp_dict['pos_length'] = pos_length
                            tmp_dict['drug_1'] = drug_1
                            tmp_dict['drug_1_pos'] = token1_pos
                            tmp_dict['drug_2'] = drug_2
                            tmp_dict['drug_2_pos'] = token2_pos
...
def run_parser(files_list_all, start_id, end_id, output_path):
    ...
    # df_file.to_csv(os.path.join(output_path,'parsed_file_{}_{}.tsv'.format(start_id+1, end_id+1)), sep='\t', index=False, encoding='utf-8') # Not saving complete file
    df_file[['pmid', 'text_@DRUG$']].sample(n=30000, random_state=1).to_csv(os.path.join(output_path,'pubmed_file_bert_data_{}_{}.tsv'.format(start_id+1, end_id+1)), sep='\t', index=False, encoding='utf-8')
    df_file.sample(n=30000, random_state=1).to_csv(os.path.join(output_path,'pubmed_file_data_{}_{}.tsv'.format(start_id+1, end_id+1)), sep='\t', index=False, encoding='utf-8')
    #30000
    del df_file
